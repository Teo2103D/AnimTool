...
def create_matched_groups_with_animation(obj_1, obj_2, start_frame, end_frame):
    ...
    # Créer le premier groupe et le matcher au premier objet
    grp_1 = cmds.group(empty=True, name="TeoOffset")
    cmds.matchTransform(grp_1, obj_1)

    # Ajouter une contrainte parent du premier groupe au premier objet
    cmds.parentConstraint(obj_1, grp_1, mo=True)

    # Créer le deuxième groupe et le matcher au deuxième objet
    grp_2 = cmds.group(empty=True, name="TeoMAT")
    cmds.matchTransform(grp_2, obj_2)

    # Faire que grp_2 soit enfant de grp_1
    cmds.parent(grp_2, grp_1)

    # Appliquer un premier matchTransform sur obj_2 à la frame start_frame
    cmds.currentTime(start_frame, edit=True)
    cmds.matchTransform(obj_2, grp_2)

    # Ajouter une clé d'animation sur le deuxième objet avant qu'il ne fasse le match avec le groupe
...
def open_ui():
    ...
    cmds.window("ConstraintAnimTool", title="ConstraintAnimTool", widthHeight=(300, 200))
